...
function NeedsScheduleByColumn(props: { package: Package }) {
  if (props.package.expectedHasDeliveryAt === null)
    return <div className="py-2 px-3">No date set.</div>

  const formattedExpectedHasDeliveryAt = DateTime.fromISO(
    props.package.expectedHasDeliveryAt,
  )

  if (!formattedExpectedHasDeliveryAt.isValid) return <div>Invalid date.</div>

  return (
    <div className="py-2 px-3">
      {formattedExpectedHasDeliveryAt.toLocaleString(DateTime.DATE_FULL)}
    </div>
  )
}
...
function PackagesTableItem({
  ...
  return (
    ...
    >
      ...
      <NeedsScheduleByColumn package={_package} />
      <div className="py-2 px-3">
        ...
        <div className="grid grid-cols-[1fr_auto]">
          <div className="flex justify-between items-center font-medium text-gray-700">
            <div>
              <p>
                Showing{" "}
                {getHumanizedOfPackageShippingType(selectedDeliveryType)}{" "}
                Packages
              </p>
            </div>
            <button
