...
function UpdateForm({ item, close }: { item: Shipment; close: () => void }) {
  ...
  const { mutate, isPending } = api.shipment.unarchiveById.useMutation({
    onSuccess: () => {
      apiUtils.shipment.delivery.getAll.invalidate()
      close()
      toast.success("Delivery unarchived.")
    },
  })
  ...
  return (
    ...
    >
      <div className="mb-3">
        Are you sure you want to unarchive this delivery?{" "}
...
export function UnarchiveModal({
  ...
  const { status, data, error } = api.shipment.delivery.getById.useQuery({
    id,
  })
