...
function UpdateForm({ item, close }: { item: Package; close: () => void }) {
  const apiUtils = api.useUtils()
  const { mutate, isPending } = api.package.unarchiveById.useMutation({
    onSuccess: () => {
  ...
  return (
    ...
    >
      <div className="mb-3">
        Are you sure you want to unarchive this package?{" "}
...
export function UnarchiveModal({
  ...
  const { status, data, error } = api.package.getById.useQuery({
    id,
  })
  return (
