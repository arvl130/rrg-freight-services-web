import type { DomesticAgent, User } from "@/server/db/entities"
import { z } from "zod"
...
export function EditForm(props: { user: User; agent: DomesticAgent | null }) {
  const {
    reset,
    register,
    formState: { errors },
    ...
    defaultValues: {
      ...
      companyName: props.agent?.companyName,
    },
    resetOptions: {
  ...
  const { isPending, mutate } = api.user.updateById.useMutation({
    onSuccess: () => {
      ...
      utils.user.getDomesticAgentDetailsById.invalidate()
    },
  })
...
export function UpdateDomesticAgentForm(props: { user: User }) {
  const { status, data, error } = api.user.getDomesticAgentDetailsById.useQuery(
    {
      id: props.user.id,
    },
  )
  ...
  if (status === "error")
    return (
      <p className="mb-3 text-red-500">
        Could not retrieve agent details: {error.message}
      </p>
    )

  return <EditForm user={props.user} agent={data} />
}
