import type { OverseasAgent, User } from "@/server/db/entities"
import { z } from "zod"
...
export function EditForm(props: { user: User; agent: OverseasAgent | null }) {
  const {
    reset,
    register,
    formState: { errors },
  ...
  const { isLoading, mutate } = api.user.updateById.useMutation({
    onSuccess: () => {
      ...
      utils.user.getOverseasAgentDetailsById.invalidate()
    },
  })
...
export function UpdateOverseasAgentForm(props: { user: User }) {
  const { status, data, error } = api.user.getOverseasAgentDetailsById.useQuery(
    {
      id: props.user.id,
    },
  )
