...
function UpdateForm({ item, close }: { item: Activity; close: () => void }) {
  const apiUtils = api.useUtils()
  const { mutate, isLoading } = api.activity.unarchiveById.useMutation({
    onSuccess: () => {
      apiUtils.activity.getAll.invalidate()
      close()
  ...
  return (
    ...
    >
      <div className="mb-3">
        Are you sure you want to unarchive this activity log?{" "}
...
export function UnarchiveModal({
  ...
  const { status, data, error } = api.activity.getById.useQuery({
    id,
  })
  return (
