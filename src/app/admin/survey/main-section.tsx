...
function TableItem({ item }: { item: Survey }) {
  ...
  return (
    <>
      <div className="px-8 py-2 border-b border-gray-300 text-sm text-left">
        {item.id}
      </div>
...
function filterBySearchTerm(items: Survey[], searchTerm: string) {
  return items.filter((item) =>
    item.id.toString().toLowerCase().includes(searchTerm),
  )
}

function filterByArchiveStatus(items: Survey[], isArchived: boolean) {
  if (isArchived) return items.filter((item) => item.isArchived === 1)

  return items.filter((item) => item.isArchived === 0)
}

function SurveyTable({ items }: { items: Survey[] }) {
      ...
      <Table.Content>
        ...
        <div className="grid grid-cols-[repeat(3,_auto)_1fr] auto-rows-min overflow-auto">
          ...
          {paginatedItems.length === 0 ? (
            <div className="text-center pt-4 col-span-7">No Survey found.</div>
          ) : (
            <>
...
export function HeaderSection() {
  return (
    <>
      <h1 className="text-2xl font-black [color:_#00203F] mb-2">Survey</h1>
    </>
  )
}

export function MainSection() {
  const { status, data: survey, error } = api.survey.getAll.useQuery()
  ...
  return <SurveyTable items={survey} />
}
